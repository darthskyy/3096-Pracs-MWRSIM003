...
// TASK 1: Create LUTs for Sine, Sawtooth, and Triangle waves
uint32_t Sin_LUT[NS] = {
...
// setting up indexing for the types of the waveforms
char* waveforms[3] = {"Sine", "Saw", "Triangle"};
uint32_t *LUT[3] = {Sin_LUT, saw_LUT, triangle_LUT};
uint32_t LUT_index = 0;

// TODO: Equation to calculate TIM2_Ticks
// TASK 3: Calculate TIM2_Ticks to generate F_SIGNAL
uint32_t TIM2_Ticks = TIM2CLK/(F_SIGNAL*NS); // How often to write new LUT value
uint32_t DestAddress = (uint32_t) &(TIM3->CCR3); // Write LUT TO TIM3->CCR3 to modify PWM duty cycle


// setting up variables for debouncing
uint32_t last_debounce_time = 0;
uint32_t debounce_delay = 500; // in ms
...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  // TASK 4: setting up the PWM and OC for TIM3 and TIM2
  ...
  HAL_DMA_Start_IT(&hdma_tim2_ch1, LUT[LUT_index], DestAddress, NS);

  // TODO: Write current waveform to LCD ("Sine")
  delay(3000);
  lcd_command(CLEAR);
  lcd_putstring(waveforms[LUT_index]);
...
void EXTI0_1_IRQHandler(void)
{
  ...
  // TASK 6: Rewrite LCD with new waveform
  lcd_command(CLEAR);
  lcd_putstring(waveforms[LUT_index]);
